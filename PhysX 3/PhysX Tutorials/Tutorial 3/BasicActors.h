...
namespace PhysicsEngine
{
	...
	public:
		...
		CompoundObject(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxVec3 pose2 = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
			GetShape(PxIdentity)->setLocalPose(PxTransform(PxVec3(pose2)));
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	};
